        ...
        while True:
            ...
            if mpu.is_pipeline_first_stage() \
                ...
                if "stop" in raw_text:
                    ...
                    mask_id = tokenizer.get_special_token('gMASK')
                    sop_id = tokenizer.get_special_token('sop')
                    context_tokens.append(mask_id)
                    context_tokens.append(sop_id)
...
def generate_samples_unconditional(model):
    ...
    context_tokens = [[tokenizer.get_special_token('gMASK'), tokenizer.get_special_token('sop')]
...
def pad_batch(batch, pad_id, args):
    # pad_length = (max([len(tokens) for tokens in batch]) + 31) // 32 * 32
    # print(pad_length)
    pad_length = args.seq_length
    context_lengths = []
    for tokens in batch:
        context_length = len(tokens)
        if context_length < pad_length:
            tokens.extend([pad_id] * (pad_length - context_length))
...
def get_token_stream(model, context_tokens):
    ...
    attention_mask = build_mask_matrix(context_length_tensor - 1, context_tokens_tensor.size(0),
